...
from shared import therds_stop, process_stop, cap_resolution, hsv_frame_queue
...
lin_dir, lin_vel, main_dir, drb_p, rot_const = 0, 0, 0, 0, 2000

main_state = 1  

start_t  = time.time()

def t_from_start(k =1000):
    return  round( (time.time() - start_t) * k)
...
if  __name__ == "__main__":  
    try:
        ...
        while True:
            ...
            if ball.main_vec.leng > 265:
                lin_dir, lin_vel = int(round(gyro - ball.main_vec.ang,3)*1000), 6000 
                main_dir, rot_const = lin_dir, 10000
                drb_p = 0

            elif main_state == 1:
                lin_dir, lin_vel = int(round(gyro - ball.main_vec.ang,3)*1000), 1200 
                main_dir, rot_const = lin_dir, 10000
                drb_p = 230
                if ball_ret:
                    main_state = 2
...
#             print(lin_vel)
            ...
            # lin_dir = round(t_from_start() % 8000 // 2000 * math.pi/2 * 1000) 
            # lin_vel = 1800 * ((t_from_start() %2000) < 1500)
            # main_dir = gyro
            # print(lin_dir, lin_vel)

            theard_serial.write_to_arduino(f',{lin_dir}, {lin_vel}, {main_dir}, {drb_p}, {rot_const},')
            # print(f',{lin_dir},{lin_vel},{main_dir},{0},{0},')
